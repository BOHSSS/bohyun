$(function(){
    ...
    function set1(){ //set객체를 만듬. 
        ...
        tim1 = setInterval( function(){ //반복실행되는 서브텍스트들을 tim으로 지정
            ...
            if(current2==subtext1.size()){ //만약 서브텍스트의 사이즈(개수)가 끝까지 갔다면
                current2=0; //초기화
            }
    ...
    $('.m_sub_text').hover( function(){ //서브텍스트에 마우스 올리면
    ...
    //모바일 메뉴
    $('.m_nav_btn').click(function(){ //햄버거버튼 클릭했을때
        $('#mo_nav').css('display','block')
    });
    $('.mo_close').click(function(){ //닫기버튼을 클릭했을때
